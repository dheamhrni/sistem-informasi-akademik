...
if (isset($_POST['cari'])){ //Mengecek apakah ada data yang dikirim melalui method POST dengan nama 'cari'
	$cari=$_POST['cari']; //Jika kondisi if terpenuhi, nilai dari $_POST['cari'] akan disimpan ke variabel $cari
	//Membuat query SQL untuk mencari data di tabel 'mhs'
	$sql="select * from mhs where nim like'%$cari%' or    
						  nama like '%$cari%' or
						  email like '%$cari%'"; //%$cari%' artinya mencari nilai yang mengandung kata yang dicari di awal, tengah, atau akhir
}else{
	$sql="select * from mhs";	//Jika tidak ada data pencarian yang dikirim, tampilkan semua data dari tabel mhs	
}
$qry = mysqli_query($koneksi,$sql) or die(mysqli_error($koneksi)); //menjalankan query yg sdh di buat
$jmlData = mysqli_num_rows($qry); //mysqli_num_rows() digunakan untuk menghitung jumlah baris hasil query
